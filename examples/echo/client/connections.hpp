...
public:
      ...
      test_times_(test_times)
  {
    ...
    service_group_.get(bas::io_service_group::io_pool).set(io_pool_size, io_pool_size);
    service_group_.get(bas::io_service_group::work_pool).set(work_pool_size, work_pool_size);
  }
  ...
  void run_once(bool force_stop)
  {
    ...
    for (std::size_t i = 0; i < connection_number_; ++i)
      client_.connect(service_group_.get(bas::io_service_group::io_pool).get_io_service(),
          service_group_.get(bas::io_service_group::work_pool).get_io_service());
...
private:
  bas::io_service_group service_group_;
  client_type client_;
